...
describe("smart-compaction", () => {
  ...
  it("skips compaction within cooldown window", async () => {
    ...
    // Hook now logs a debug message and returns void (no skip return since v2026.5.2)
  ...
  it("skips compaction when too few new messages", async () => {
    ...
    // Only 3 new messages since last compaction — hook logs debug and returns void
  ...
  it("tracks separate sessions independently", async () => {
    ...
    // sess-A is in cooldown → hook logs and returns void (no skip support since v2026.5.2)
