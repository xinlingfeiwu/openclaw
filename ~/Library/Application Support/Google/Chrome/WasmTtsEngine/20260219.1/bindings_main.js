...
const loadWasmTtsBindings = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    (function () {
      ...
      var currentNodeVersion = isNode
        ? humanReadableVersionToPacked(process.versions.node)
        : TARGET_NOT_SUPPORTED;
      if (currentNodeVersion < 160400) {
        throw new Error(
          `This emscripten-generated code requires node v${packedVersionToHumanReadable(160400)} (detected v${packedVersionToHumanReadable(currentNodeVersion)})`,
        );
      }
    ...
    var IDBFS = {
      ...
      reconcile: (src, dst, callback) => {
        ...
        for (const path of create.toSorted()) {
          if (dst.type === "local") {
        ...
        // parent directories
        for (var path of remove.toSorted().toReversed()) {
          if (dst.type === "local") {
    ...
    var FS = {
      ...
      createPath(parent, path, canRead, canWrite) {
        ...
        var parts = path.split("/").toReversed();
        while (parts.length) {
          var part = parts.pop();
          if (!part) {
            continue;
    ...
    var waitAsyncPolyfilled =
      ...
      (globalThis.navigator?.userAgent &&
        Number((navigator.userAgent.match(/Chrom(e|ium)\/([0-9]+)\./) || [])[2]) < 91);
    ...
    var __emscripten_receive_on_main_thread_js = (
      ...
      assert(
        func.length == proxiedJSCallArgs.length,
    ...
    function __munmap_js(addr, len, prot, flags, fd, offset_low, offset_high) {
      if (ENVIRONMENT_IS_PTHREAD) {
        return proxyToMainThread(15, 0, 1, addr, len, prot, flags, fd, offset_low, offset_high);
      }
      var offset = convertI32PairToI53Checked(offset_low, offset_high);
      try {
        var stream = SYSCALLS.getStreamFromFD(fd);
    ...
    var _emscripten_num_logical_cores = () =>
      ENVIRONMENT_IS_NODE ? require("node:os").cpus().length : navigator["hardwareConcurrency"];
    ...
    function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
      if (ENVIRONMENT_IS_PTHREAD) {
        return proxyToMainThread(20, 0, 1, fd, offset_low, offset_high, whence, newOffset);
      }
      var offset = convertI32PairToI53Checked(offset_low, offset_high);
      try {
        if (isNaN(offset)) {
    ...
    var _GoogleTtsShutdown = (Module["_GoogleTtsShutdown"] =
      makeInvalidEarlyAccess("_GoogleTtsShutdown"));

    var _GoogleTtsInstallVoice = (Module["_GoogleTtsInstallVoice"] =
      makeInvalidEarlyAccess("_GoogleTtsInstallVoice"));

    var _GoogleTtsInitBuffered = (Module["_GoogleTtsInitBuffered"] =
      makeInvalidEarlyAccess("_GoogleTtsInitBuffered"));

    var _GoogleTtsReadBuffered = (Module["_GoogleTtsReadBuffered"] =
      makeInvalidEarlyAccess("_GoogleTtsReadBuffered"));

    var _GoogleTtsFinalizeBuffered = (Module["_GoogleTtsFinalizeBuffered"] = makeInvalidEarlyAccess(
      "_GoogleTtsFinalizeBuffered",
    ));

    var _GoogleTtsGetTimepointsCount = (Module["_GoogleTtsGetTimepointsCount"] =
      makeInvalidEarlyAccess("_GoogleTtsGetTimepointsCount"));

    var _GoogleTtsGetTimepointsTimeInSecsAtIndex = (Module[
      "_GoogleTtsGetTimepointsTimeInSecsAtIndex"
    ] = makeInvalidEarlyAccess("_GoogleTtsGetTimepointsTimeInSecsAtIndex"));

    var _GoogleTtsGetTimepointsCharIndexAtIndex = (Module[
      "_GoogleTtsGetTimepointsCharIndexAtIndex"
    ] = makeInvalidEarlyAccess("_GoogleTtsGetTimepointsCharIndexAtIndex"));

    var _GoogleTtsGetTimepointsCharLengthAtIndex = (Module[
      "_GoogleTtsGetTimepointsCharLengthAtIndex"
    ] = makeInvalidEarlyAccess("_GoogleTtsGetTimepointsCharLengthAtIndex"));

    var _GoogleTtsGetEventBufferPtr = (Module["_GoogleTtsGetEventBufferPtr"] =
      makeInvalidEarlyAccess("_GoogleTtsGetEventBufferPtr"));

    var _GoogleTtsGetEventBufferLen = (Module["_GoogleTtsGetEventBufferLen"] =
      makeInvalidEarlyAccess("_GoogleTtsGetEventBufferLen"));
    ...
    var _kVersionStampBuildChangelistStr = (Module["_kVersionStampBuildChangelistStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildChangelistStr"));

    var _kVersionStampCitcSnapshotStr = (Module["_kVersionStampCitcSnapshotStr"] =
      makeInvalidEarlyAccess("_kVersionStampCitcSnapshotStr"));

    var _kVersionStampCitcWorkspaceIdStr = (Module["_kVersionStampCitcWorkspaceIdStr"] =
      makeInvalidEarlyAccess("_kVersionStampCitcWorkspaceIdStr"));

    var _kVersionStampSourceUriStr = (Module["_kVersionStampSourceUriStr"] = makeInvalidEarlyAccess(
      "_kVersionStampSourceUriStr",
    ));

    var _kVersionStampBuildClientStr = (Module["_kVersionStampBuildClientStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildClientStr"));

    var _kVersionStampBuildClientMintStatusStr = (Module["_kVersionStampBuildClientMintStatusStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildClientMintStatusStr"));

    var _kVersionStampBuildCompilerStr = (Module["_kVersionStampBuildCompilerStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildCompilerStr"));

    var _kVersionStampBuildDateTimePstStr = (Module["_kVersionStampBuildDateTimePstStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildDateTimePstStr"));
    ...
    var _kVersionStampBuildInfoStr = (Module["_kVersionStampBuildInfoStr"] = makeInvalidEarlyAccess(
      "_kVersionStampBuildInfoStr",
    ));

    var _kVersionStampBuildLabelStr = (Module["_kVersionStampBuildLabelStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildLabelStr"));

    var _kVersionStampBuildTargetStr = (Module["_kVersionStampBuildTargetStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildTargetStr"));

    var _kVersionStampBuildTimestampStr = (Module["_kVersionStampBuildTimestampStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildTimestampStr"));

    var _kVersionStampBuildToolStr = (Module["_kVersionStampBuildToolStr"] = makeInvalidEarlyAccess(
      "_kVersionStampBuildToolStr",
    ));

    var _kVersionStampG3BuildTargetStr = (Module["_kVersionStampG3BuildTargetStr"] =
      makeInvalidEarlyAccess("_kVersionStampG3BuildTargetStr"));

    var _kVersionStampVerifiableStr = (Module["_kVersionStampVerifiableStr"] =
      makeInvalidEarlyAccess("_kVersionStampVerifiableStr"));

    var _kVersionStampBuildFdoTypeStr = (Module["_kVersionStampBuildFdoTypeStr"] =
    ...
    ] = makeInvalidEarlyAccess("_kVersionStampBuildBaselineChangelistStr"));

    var _kVersionStampBuildLtoTypeStr = (Module["_kVersionStampBuildLtoTypeStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildLtoTypeStr"));

    var _kVersionStampBuildPropellerTypeStr = (Module["_kVersionStampBuildPropellerTypeStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildPropellerTypeStr"));

    var _kVersionStampBuildPghoTypeStr = (Module["_kVersionStampBuildPghoTypeStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildPghoTypeStr"));

    var _kVersionStampBuildUsernameStr = (Module["_kVersionStampBuildUsernameStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildUsernameStr"));

    var _kVersionStampBuildHostnameStr = (Module["_kVersionStampBuildHostnameStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildHostnameStr"));

    var _kVersionStampBuildDirectoryStr = (Module["_kVersionStampBuildDirectoryStr"] =
      makeInvalidEarlyAccess("_kVersionStampBuildDirectoryStr"));

    var _kVersionStampBuildChangelistInt = (Module["_kVersionStampBuildChangelistInt"] =
      makeInvalidEarlyAccess("_kVersionStampBuildChangelistInt"));

    var _kVersionStampCitcSnapshotInt = (Module["_kVersionStampCitcSnapshotInt"] =
      makeInvalidEarlyAccess("_kVersionStampCitcSnapshotInt"));
    ...
    function assignWasmExports(wasmExports) {
      ...
      assert(
        typeof wasmExports["GoogleTtsInit"] != "undefined",
        "missing Wasm export: GoogleTtsInit",
      );
      assert(
        typeof wasmExports["GoogleTtsShutdown"] != "undefined",
        "missing Wasm export: GoogleTtsShutdown",
      );
      assert(
        typeof wasmExports["GoogleTtsInstallVoice"] != "undefined",
        "missing Wasm export: GoogleTtsInstallVoice",
      );
      assert(
        typeof wasmExports["GoogleTtsInitBuffered"] != "undefined",
        "missing Wasm export: GoogleTtsInitBuffered",
      );
      assert(
        typeof wasmExports["GoogleTtsReadBuffered"] != "undefined",
        "missing Wasm export: GoogleTtsReadBuffered",
      );
      assert(
        typeof wasmExports["GoogleTtsFinalizeBuffered"] != "undefined",
        "missing Wasm export: GoogleTtsFinalizeBuffered",
      );
      assert(
        typeof wasmExports["GoogleTtsGetTimepointsCount"] != "undefined",
        "missing Wasm export: GoogleTtsGetTimepointsCount",
      );
      assert(
        typeof wasmExports["GoogleTtsGetTimepointsTimeInSecsAtIndex"] != "undefined",
        "missing Wasm export: GoogleTtsGetTimepointsTimeInSecsAtIndex",
      );
      assert(
        typeof wasmExports["GoogleTtsGetTimepointsCharIndexAtIndex"] != "undefined",
        "missing Wasm export: GoogleTtsGetTimepointsCharIndexAtIndex",
      );
      assert(
        typeof wasmExports["GoogleTtsGetTimepointsCharLengthAtIndex"] != "undefined",
        "missing Wasm export: GoogleTtsGetTimepointsCharLengthAtIndex",
      );
      assert(
        typeof wasmExports["GoogleTtsGetEventBufferPtr"] != "undefined",
        "missing Wasm export: GoogleTtsGetEventBufferPtr",
      );
      assert(
        typeof wasmExports["GoogleTtsGetEventBufferLen"] != "undefined",
        "missing Wasm export: GoogleTtsGetEventBufferLen",
      );
      ...
      assert(
        typeof wasmExports["_embind_initialize_bindings"] != "undefined",
        "missing Wasm export: _embind_initialize_bindings",
      );
      assert(
      ...
      assert(
        typeof wasmExports["_emscripten_run_js_on_main_thread_done"] != "undefined",
        "missing Wasm export: _emscripten_run_js_on_main_thread_done",
      );
      assert(
      ...
      assert(
        typeof wasmExports["dynCall_iiiijij"] != "undefined",
        "missing Wasm export: dynCall_iiiijij",
      );
      assert(
        typeof wasmExports["dynCall_jiji"] != "undefined",
        "missing Wasm export: dynCall_jiji",
      );
      assert(
        typeof wasmExports["dynCall_vijj"] != "undefined",
        "missing Wasm export: dynCall_vijj",
      );
      ...
      assert(
        typeof wasmExports["dynCall_viiiijii"] != "undefined",
        "missing Wasm export: dynCall_viiiijii",
      );
      assert(
        typeof wasmExports["dynCall_jiiii"] != "undefined",
        "missing Wasm export: dynCall_jiiii",
      );
      assert(
        typeof wasmExports["dynCall_jiii"] != "undefined",
        "missing Wasm export: dynCall_jiii",
      );
      assert(
        typeof wasmExports["dynCall_viij"] != "undefined",
        "missing Wasm export: dynCall_viij",
      );
      assert(
        typeof wasmExports["dynCall_viijii"] != "undefined",
        "missing Wasm export: dynCall_viijii",
      );
      assert(typeof wasmExports["dynCall_jii"] != "undefined", "missing Wasm export: dynCall_jii");
      assert(
        typeof wasmExports["dynCall_jiij"] != "undefined",
        "missing Wasm export: dynCall_jiij",
      );
      ...
      assert(
        typeof wasmExports["dynCall_iiiijj"] != "undefined",
        "missing Wasm export: dynCall_iiiijj",
      );
      assert(
        typeof wasmExports["dynCall_viijj"] != "undefined",
        "missing Wasm export: dynCall_viijj",
      );
      assert(
        typeof wasmExports["dynCall_viiijjj"] != "undefined",
        "missing Wasm export: dynCall_viiijjj",
      );
      assert(
        typeof wasmExports["dynCall_iiij"] != "undefined",
        "missing Wasm export: dynCall_iiij",
      );
      assert(
        typeof wasmExports["dynCall_jiijj"] != "undefined",
        "missing Wasm export: dynCall_jiijj",
      );
      assert(
        typeof wasmExports["dynCall_viji"] != "undefined",
        "missing Wasm export: dynCall_viji",
      );
      assert(
        typeof wasmExports["dynCall_iiji"] != "undefined",
        "missing Wasm export: dynCall_iiji",
      );
      assert(
        typeof wasmExports["dynCall_iijjiii"] != "undefined",
        "missing Wasm export: dynCall_iijjiii",
      );
      assert(
        typeof wasmExports["dynCall_vijjjii"] != "undefined",
        "missing Wasm export: dynCall_vijjjii",
      );
      assert(
        typeof wasmExports["dynCall_vijjj"] != "undefined",
        "missing Wasm export: dynCall_vijjj",
      );
      assert(typeof wasmExports["dynCall_vj"] != "undefined", "missing Wasm export: dynCall_vj");
      assert(
        typeof wasmExports["dynCall_iijjiiii"] != "undefined",
        "missing Wasm export: dynCall_iijjiiii",
      );
      assert(
        typeof wasmExports["dynCall_iiiiij"] != "undefined",
        "missing Wasm export: dynCall_iiiiij",
      );
      assert(
      ...
      assert(
        typeof wasmExports["kVersionStampBuildChangelistStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildChangelistStr",
      );
      assert(
        typeof wasmExports["kVersionStampCitcSnapshotStr"] != "undefined",
        "missing Wasm export: kVersionStampCitcSnapshotStr",
      );
      assert(
        typeof wasmExports["kVersionStampCitcWorkspaceIdStr"] != "undefined",
        "missing Wasm export: kVersionStampCitcWorkspaceIdStr",
      );
      assert(
        typeof wasmExports["kVersionStampSourceUriStr"] != "undefined",
        "missing Wasm export: kVersionStampSourceUriStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildClientStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildClientStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildClientMintStatusStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildClientMintStatusStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildCompilerStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildCompilerStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildDateTimePstStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildDateTimePstStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildDepotPathStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildDepotPathStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildIdStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildIdStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildInfoStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildInfoStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildLabelStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildLabelStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildTargetStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildTargetStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildTimestampStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildTimestampStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildToolStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildToolStr",
      );
      assert(
        typeof wasmExports["kVersionStampG3BuildTargetStr"] != "undefined",
        "missing Wasm export: kVersionStampG3BuildTargetStr",
      );
      assert(
        typeof wasmExports["kVersionStampVerifiableStr"] != "undefined",
        "missing Wasm export: kVersionStampVerifiableStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildFdoTypeStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildFdoTypeStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildBaselineChangelistStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildBaselineChangelistStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildLtoTypeStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildLtoTypeStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildPropellerTypeStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildPropellerTypeStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildPghoTypeStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildPghoTypeStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildUsernameStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildUsernameStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildHostnameStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildHostnameStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildDirectoryStr"] != "undefined",
        "missing Wasm export: kVersionStampBuildDirectoryStr",
      );
      assert(
        typeof wasmExports["kVersionStampBuildChangelistInt"] != "undefined",
        "missing Wasm export: kVersionStampBuildChangelistInt",
      );
      assert(
        typeof wasmExports["kVersionStampCitcSnapshotInt"] != "undefined",
        "missing Wasm export: kVersionStampCitcSnapshotInt",
      );
      assert(
        typeof wasmExports["kVersionStampBuildClientMintStatusInt"] != "undefined",
        "missing Wasm export: kVersionStampBuildClientMintStatusInt",
      );
      assert(
        typeof wasmExports["kVersionStampBuildTimestampInt"] != "undefined",
        "missing Wasm export: kVersionStampBuildTimestampInt",
      );
      assert(
        typeof wasmExports["kVersionStampVerifiableInt"] != "undefined",
        "missing Wasm export: kVersionStampVerifiableInt",
      );
      assert(
        typeof wasmExports["kVersionStampBuildCoverageEnabledInt"] != "undefined",
        "missing Wasm export: kVersionStampBuildCoverageEnabledInt",
      );
      assert(
        typeof wasmExports["kVersionStampBuildBaselineChangelistInt"] != "undefined",
        "missing Wasm export: kVersionStampBuildBaselineChangelistInt",
      );
      assert(
        typeof wasmExports["kVersionStampPrecookedTimestampStr"] != "undefined",
        "missing Wasm export: kVersionStampPrecookedTimestampStr",
      );
      assert(
        typeof wasmExports["kVersionStampPrecookedClientInfoStr"] != "undefined",
        "missing Wasm export: kVersionStampPrecookedClientInfoStr",
      );
      assert(
      ...
      _GoogleTtsShutdown = Module["_GoogleTtsShutdown"] = createExportWrapper(
        "GoogleTtsShutdown",
        0,
      );
      _GoogleTtsInstallVoice = Module["_GoogleTtsInstallVoice"] = createExportWrapper(
        "GoogleTtsInstallVoice",
        3,
      );
      _GoogleTtsInitBuffered = Module["_GoogleTtsInitBuffered"] = createExportWrapper(
        "GoogleTtsInitBuffered",
        4,
      );
      _GoogleTtsReadBuffered = Module["_GoogleTtsReadBuffered"] = createExportWrapper(
        "GoogleTtsReadBuffered",
        0,
      );
      _GoogleTtsFinalizeBuffered = Module["_GoogleTtsFinalizeBuffered"] = createExportWrapper(
        "GoogleTtsFinalizeBuffered",
        0,
      );
      _GoogleTtsGetTimepointsCount = Module["_GoogleTtsGetTimepointsCount"] = createExportWrapper(
        "GoogleTtsGetTimepointsCount",
        0,
      );
      _GoogleTtsGetTimepointsTimeInSecsAtIndex = Module[
        "_GoogleTtsGetTimepointsTimeInSecsAtIndex"
      ] = createExportWrapper("GoogleTtsGetTimepointsTimeInSecsAtIndex", 1);
      _GoogleTtsGetTimepointsCharIndexAtIndex = Module["_GoogleTtsGetTimepointsCharIndexAtIndex"] =
        createExportWrapper("GoogleTtsGetTimepointsCharIndexAtIndex", 1);
      _GoogleTtsGetTimepointsCharLengthAtIndex = Module[
        "_GoogleTtsGetTimepointsCharLengthAtIndex"
      ] = createExportWrapper("GoogleTtsGetTimepointsCharLengthAtIndex", 1);
      _GoogleTtsGetEventBufferPtr = Module["_GoogleTtsGetEventBufferPtr"] = createExportWrapper(
        "GoogleTtsGetEventBufferPtr",
        0,
      );
      _GoogleTtsGetEventBufferLen = Module["_GoogleTtsGetEventBufferLen"] = createExportWrapper(
      ...
      _kVersionStampBuildChangelistStr = Module["_kVersionStampBuildChangelistStr"] =
        wasmExports["kVersionStampBuildChangelistStr"].value;
      _kVersionStampCitcSnapshotStr = Module["_kVersionStampCitcSnapshotStr"] =
        wasmExports["kVersionStampCitcSnapshotStr"].value;
      _kVersionStampCitcWorkspaceIdStr = Module["_kVersionStampCitcWorkspaceIdStr"] =
        wasmExports["kVersionStampCitcWorkspaceIdStr"].value;
      _kVersionStampSourceUriStr = Module["_kVersionStampSourceUriStr"] =
        wasmExports["kVersionStampSourceUriStr"].value;
      _kVersionStampBuildClientStr = Module["_kVersionStampBuildClientStr"] =
        wasmExports["kVersionStampBuildClientStr"].value;
      _kVersionStampBuildClientMintStatusStr = Module["_kVersionStampBuildClientMintStatusStr"] =
        wasmExports["kVersionStampBuildClientMintStatusStr"].value;
      _kVersionStampBuildCompilerStr = Module["_kVersionStampBuildCompilerStr"] =
        wasmExports["kVersionStampBuildCompilerStr"].value;
      _kVersionStampBuildDateTimePstStr = Module["_kVersionStampBuildDateTimePstStr"] =
        wasmExports["kVersionStampBuildDateTimePstStr"].value;
      _kVersionStampBuildDepotPathStr = Module["_kVersionStampBuildDepotPathStr"] =
        wasmExports["kVersionStampBuildDepotPathStr"].value;
      _kVersionStampBuildIdStr = Module["_kVersionStampBuildIdStr"] =
        wasmExports["kVersionStampBuildIdStr"].value;
      _kVersionStampBuildInfoStr = Module["_kVersionStampBuildInfoStr"] =
        wasmExports["kVersionStampBuildInfoStr"].value;
      _kVersionStampBuildLabelStr = Module["_kVersionStampBuildLabelStr"] =
        wasmExports["kVersionStampBuildLabelStr"].value;
      _kVersionStampBuildTargetStr = Module["_kVersionStampBuildTargetStr"] =
        wasmExports["kVersionStampBuildTargetStr"].value;
      _kVersionStampBuildTimestampStr = Module["_kVersionStampBuildTimestampStr"] =
        wasmExports["kVersionStampBuildTimestampStr"].value;
      _kVersionStampBuildToolStr = Module["_kVersionStampBuildToolStr"] =
        wasmExports["kVersionStampBuildToolStr"].value;
      _kVersionStampG3BuildTargetStr = Module["_kVersionStampG3BuildTargetStr"] =
        wasmExports["kVersionStampG3BuildTargetStr"].value;
      _kVersionStampVerifiableStr = Module["_kVersionStampVerifiableStr"] =
        wasmExports["kVersionStampVerifiableStr"].value;
      _kVersionStampBuildFdoTypeStr = Module["_kVersionStampBuildFdoTypeStr"] =
        wasmExports["kVersionStampBuildFdoTypeStr"].value;
      _kVersionStampBuildBaselineChangelistStr = Module[
        "_kVersionStampBuildBaselineChangelistStr"
      ] = wasmExports["kVersionStampBuildBaselineChangelistStr"].value;
      _kVersionStampBuildLtoTypeStr = Module["_kVersionStampBuildLtoTypeStr"] =
        wasmExports["kVersionStampBuildLtoTypeStr"].value;
      _kVersionStampBuildPropellerTypeStr = Module["_kVersionStampBuildPropellerTypeStr"] =
        wasmExports["kVersionStampBuildPropellerTypeStr"].value;
      _kVersionStampBuildPghoTypeStr = Module["_kVersionStampBuildPghoTypeStr"] =
        wasmExports["kVersionStampBuildPghoTypeStr"].value;
      _kVersionStampBuildUsernameStr = Module["_kVersionStampBuildUsernameStr"] =
        wasmExports["kVersionStampBuildUsernameStr"].value;
      _kVersionStampBuildHostnameStr = Module["_kVersionStampBuildHostnameStr"] =
        wasmExports["kVersionStampBuildHostnameStr"].value;
      _kVersionStampBuildDirectoryStr = Module["_kVersionStampBuildDirectoryStr"] =
        wasmExports["kVersionStampBuildDirectoryStr"].value;
      _kVersionStampBuildChangelistInt = Module["_kVersionStampBuildChangelistInt"] =
        wasmExports["kVersionStampBuildChangelistInt"].value;
      _kVersionStampCitcSnapshotInt = Module["_kVersionStampCitcSnapshotInt"] =
        wasmExports["kVersionStampCitcSnapshotInt"].value;
      ...
      _kVersionStampBuildTimestampInt = Module["_kVersionStampBuildTimestampInt"] =
        wasmExports["kVersionStampBuildTimestampInt"].value;
      _kVersionStampVerifiableInt = Module["_kVersionStampVerifiableInt"] =
        wasmExports["kVersionStampVerifiableInt"].value;
      _kVersionStampBuildCoverageEnabledInt = Module["_kVersionStampBuildCoverageEnabledInt"] =
        wasmExports["kVersionStampBuildCoverageEnabledInt"].value;
      _kVersionStampBuildBaselineChangelistInt = Module[
        "_kVersionStampBuildBaselineChangelistInt"
      ] = wasmExports["kVersionStampBuildBaselineChangelistInt"].value;
      _kVersionStampPrecookedTimestampStr = Module["_kVersionStampPrecookedTimestampStr"] =
        wasmExports["kVersionStampPrecookedTimestampStr"].value;
      _kVersionStampPrecookedClientInfoStr = Module["_kVersionStampPrecookedClientInfoStr"] =
        wasmExports["kVersionStampPrecookedClientInfoStr"].value;
...
let isPthread = globalThis.self?.name?.startsWith("em-pthread");
// In order to support both web and node we also need to detect node here.
const isNode = globalThis.process?.versions?.node && globalThis.process?.type != "renderer";
if (isNode) {
  isPthread = require("node:worker_threads").workerData === "em-pthread";
}
