...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Lazily load the langfuse SDK via dynamic import to avoid bundler static analysis.
    // This keeps langfuse as a truly optional runtime dependency.
    const langfusePkg = "langfuse";
    let langfusePromise: Promise<LangfuseInstance | null> | null = null;
    function getLangfuse(): Promise<LangfuseInstance | null> {
      if (!langfusePromise) {
        langfusePromise = (import(langfusePkg) as Promise<Record<string, unknown>>)
          .then((mod) => {
