...
async function resolveCollectionId(
  ...
  if (!match) {
    throw new Error(
      `ChromaDB collection "${collectionName}" not found. Available: ${collections.map((c) => c.name).join(", ")}`,
    );
  }
...
async function queryChromaDBHybrid(
  ...
  for (const kw of keywords.slice(0, 3)) {
    // Limit to 3 keywords
    try {
      const kwResults = await queryChromaDBRaw(chromaUrl, collectionId, embedding, nResults, {
        $contains: kw,
      });
  ...
  return [...seen.values()].sort((a, b) => b.score - a.score).slice(0, nResults);
}
...
export default function register(api: OpenClawPluginApi) {
  ...
  if (!cfg.collectionId && !cfg.collectionName) {
    api.logger.warn(
      "chromadb-memory: No collectionId or collectionName configured, plugin disabled",
    );
    return;
  }
  ...
  api.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      try {
        ...
        const embedding = await getEmbedding(cfg.ollamaUrl, cfg.embeddingModel, query);
        ...
        if (results.length === 0) {
          return {
            content: [{ type: "text", text: "No relevant results found in ChromaDB." }],
          };
        }
  ...
  if (cfg.autoRecall) {
    api.on("before_agent_start", async (event: { prompt?: string }) => {
      ...
      try {
        const collectionId = await getCollectionId();
        const embedding = await getEmbedding(cfg.ollamaUrl, cfg.embeddingModel, event.prompt);
        const results = await queryChromaDBHybrid(
          cfg.chromaUrl,
          collectionId,
          embedding,
        ...
        const memoryContext = relevant
          .map((r) => `- [${r.source}] ${r.text.slice(0, 300)}${r.text.length > 300 ? "..." : ""}`)
          .join("\n");
        ...
        api.logger.warn(
          `chromadb-memory: auto-recall failed (${_consecutiveFailures}x): ${errMsg}`,
        );
