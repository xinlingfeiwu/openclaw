...
class ConnectionManager {
  ...
  async getConnection(port) {
    if (
      ...
      this.connections[port].ws.readyState === 1
    ) {
    ...
    const browserUrl = await new Promise((resolve, reject) => {
      http
        .get(`http://127.0.0.1:${port}/json/version`, (r) => {
          let d = "";
          r.on("data", (c) => (d += c));
          r.on("end", () => {
            try {
...
class Browser {
  ...
  async tabs() {
    ...
    const doHttp = () =>
      new Promise((resolve, reject) => {
        http
          .get(`http://127.0.0.1:${this.port}/json/list`, (r) => {
            let d = "";
            r.on("data", (c) => (d += c));
            r.on("end", () => {
              try {
  ...
  async newTab(url) {
    ...
    const browserInfo = await new Promise((res, rej) => {
      http
        .get(`http://127.0.0.1:${this.port}/json/version`, (r) => {
          let d = "";
          r.on("data", (c) => (d += c));
          r.on("end", () => {
            try {
    ...
    const tabId = await new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(new Error("createTarget timeout"));
      }, 15000);
      const handler = (data) => {
        const msg = JSON.parse(data.toString());
        if (msg.id === id && msg.result?.targetId) {
          clearTimeout(timeout);
