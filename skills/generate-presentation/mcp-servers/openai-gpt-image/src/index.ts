...
(async () => {
  ...
  const createImageSchema = z
    ...
    .refine(
      ...
      {
        message: "file_output must be an absolute path when output is 'file_output'",
        path: ["file_output"],
      },
    );
  ...
  server.tool(
    ...
    async (args, _extra) => {
      ...
      if (
        ...
        output_format &&
        !["png", "webp"].includes(output_format)
      ) {
      ...
      const totalBase64Size = images.reduce(
      ...
      if (output === "base64" && totalBase64Size > MAX_RESPONSE_SIZE) {
        ...
        if (!file_output) {
          ...
          effectiveFileOutput = path.join(
            tmpDir,
            `openai_image_${unique}.${images[0]?.ext ?? "png"}`,
          );
        }
      }
  ...
  const base64Check = (val: string | undefined) =>
    !!val && (/^([A-Za-z0-9+/=\r\n]+)$/.test(val) || val.startsWith("data:image/"));
  const imageInputSchema = z
    .string()
    .refine((val) => absolutePathCheck(val) || base64Check(val), {
      message: "Must be an absolute path or a base64-encoded string (optionally as a data URL)",
    })
    .describe(
      "Absolute path to an image file (png, jpg, webp < 25MB) or a base64-encoded image string.",
    );
  ...
  const editImageBaseSchema = z.object({
    ...
    prompt: z
      .string()
      .max(32000)
      .describe("A text description of the desired edit. Max 32000 chars."),
    mask: z
      .string()
      .optional()
      .describe(
    ...
    quality: z
      ...
      .optional()
      .describe("Quality (high, medium, low) - only for gpt-image-1."),
    size: z
    ...
    output: z
      ...
      .default("base64")
      .describe("Output format: base64 or file path."),
    file_output: z
      .string()
      .refine(absolutePathCheck, { message: "Path must be absolute" })
      .optional()
      .describe(
  ...
  server.tool(
    ...
    async (args, _extra) => {
      ...
      if (!absolutePathCheck(validatedArgs.image) && !base64Check(validatedArgs.image)) {
        throw new Error(
          "Invalid 'image' input: Must be an absolute path or a base64-encoded string.",
        );
      }
      if (
        validatedArgs.mask &&
        !absolutePathCheck(validatedArgs.mask) &&
        !base64Check(validatedArgs.mask)
      ) {
        throw new Error(
          "Invalid 'mask' input: Must be an absolute path or a base64-encoded string.",
        );
      }
      ...
      async function inputToFile(input: string, idx = 0) {
        if (absolutePathCheck(input)) {
          ...
          return await toFile(buffer, `input_${idx}.${mime.split("/")[1] || "png"}`, {
            type: mime,
          });
        }
      }
