        ...
        "requires":
          {
            ...
            "env":
              {
                "TOPIC_MONITOR_TELEGRAM_ID": "optional - Telegram chat ID for alerts",
                "TOPIC_MONITOR_DATA_DIR": "optional - defaults to .data/ in skill dir",
                "WEB_SEARCH_PLUS_PATH": "optional - defaults to relative path",
              },
            "note": "All env vars optional. Defaults work out of the box.",
          },
      },
  }
---
...
```json
{
  "alert_on": ["price_change_10pct", "keyword_exact_match", "source_tier_1", "high_engagement"],
  "ignore_sources": ["spam-site.com", "clickbait-news.io"],
  "boost_sources": ["github.com", "arxiv.org", "official-site.com"]
}
```
...
```json
{
  "patterns": ["version \\d+\\.\\d+\\.\\d+", "\\$\\d{1,3}(,\\d{3})*", "CVE-\\d{4}-\\d+"]
}
```
...
```json
{
  "topics": {
    "eth-price": {
      ...
      "alerted_urls": ["https://example.com/eth-news-1"],
      "findings_count": 3,
      "alerts_today": 1
    }
  },
